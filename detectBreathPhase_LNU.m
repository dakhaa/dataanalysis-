function [breathInfo, threshCrossings, breathPhase, thresholdSig, Breath_rate_FFT] = ...
            detectBreathPhase_LNU(impedanceSig, fSampling, fRespRange, doPlot)
...
%%% Short-time Fourier Transform, settings %%%
        ...
        Nfft=1024;              % Size of zero padded FFT
        kstop=round(250/(60*fs/Nfft)); % Max bin number of ST
        twin=4;                 % window time for frequency analysis                 
        Nwin=round(twin/T);     % window length in samples
        beta=4;
        h=kaiser(Nwin,beta);         % Kaiser window with parameter beta
        % Highpass filter for FFT analysis
        Wn=2*[15]/(60*fs);   % highpass filter
        [B,A]=butter(2,Wn,'high');         % Butterworth filter
        impedanceSig_HP=filter(B,A,impedanceSig);% HP filtered sum signal for FFT analysis
        Breath_rate_FFT=zeros(length(impedanceSig),1);
...
for i = 3 : length(impedanceSig)-2*fSampling
  ...
  %  if ( mean(range) > 0)
        ...
        if i>=Nwin
        x=impedanceSig_HP(i-Nwin+1:i);
        Xfft=abs(fft(h.*(x-mean(x)),Nfft));    % FFT with Kaiser window
        [ymax kmax]=max(Xfft(1:kstop));
        Breath_rate_FFT(i)=(kmax-1)*(fs/Nfft)*60;
        end
  ...
  %% simple state machine (rising/falling) crossing
        if waitForRisingCrossing
            ...
            %%% STFT-algorithm %%%
            MINIMAL_IDENTICAL_CROSSING_SPACING = max(2,ceil(1/2*(fs*60/Breath_rate_FFT(i))));
            MINIMAL_DIFFERENT_CROSSING_SPACING = max(1,ceil(1/4*(fs*60/Breath_rate_FFT(i))));
            %%%%%%%%%%%%%%%%%%%%%%
    ...
    %             end
                ...
                if ~((iBreath == 0) && (breathInfo(1, 1) == 0))
                    iBreath = iBreath + 1;
                    breathInfo(2:3,iBreath) = [currentMaximumPosition, currentMinimumPosition-1]; %%%%Davood
                end
